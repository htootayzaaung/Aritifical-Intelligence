...
# ------------------ Group Submission By Arjun Krishnan (sc21ak) and Htoo Aung (sc21hta) ----------------------------
# |                                                                                                                 |
# |                                                                                                                 |
# -------------------------------------------------------------------------------------------------------------------
...
def deviation(value,parent_pos,parent_neg):
    ...
    #Insert code here
    deviation = (((value.pos - (parent_pos*((value.pos + value.neg)/(parent_pos + parent_neg))))**2 /(parent_pos*((value.pos + value.neg)/(parent_pos + parent_neg)))) + 
    ((value.neg - (parent_neg*((value.pos + value.neg)/(parent_pos + parent_neg))))**2 / (parent_neg*((value.pos + value.neg)/(parent_pos + parent_neg)))))
...
def test_tennis_tree(filename):
    ...
    trainSet,testSet = train_test_split(mydataset, test_split=0.2)
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    #insert code here
    fullTree = DecisionTreeLearner(trainSet)
    error = err_ratio(fullTree,testSet)
    errorFound = False
    for i in range(0,N):
        split = 1-((i+1)/200)
        trainset,notUsedTestSet = train_test_split(trainSet,test_split=split)
        tree = DecisionTreeLearner(trainset)
        err = err_ratio(tree,testSet)
        if (err == error and not errorFound):
            errorFound = True
            samples_required = i
